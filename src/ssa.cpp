...
void Block::find_defs()
{
    for (Instruction& in : instr)
        if (in.op == Opcode::MOVE && !in.oper2.tag.empty())
            defs.insert(in.oper2.tag);
}
...
void Block::ssa_rename_var(map<string, RenameStack>& stack)
{
    ...
    for (auto& in : instr) {
        if (!in.oper1.tag.empty())
            in.oper1.ssa_idx = stack[in.oper1.tag].top();
        if (!in.oper2.tag.empty())
            in.oper2.ssa_idx = stack[in.oper2.tag].top();

        if (in.op == Opcode::MOVE && !in.oper2.tag.empty())
            in.oper2.ssa_idx = stack[in.oper2.tag].push();
    }
    ...
    for (Function* func : funcs) {
        ...
        for (auto& addr_block : func->blocks) {
            ...
            if (in.op == Opcode::BR)
                ...
                fprintf(out, "instr %lld: blbc (%lld) [%lld]\n", in.addr, in.oper1.value, block->br_next->ssa_addr);
